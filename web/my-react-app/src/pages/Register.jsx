  ...
  const handleContinue =  (e) => {
    e.preventDefault();
      ...
      const handleSubmit = async (e) => {
        ...
        let user = {
          email, password, type: userType, name,
        };
...
setError(res.data.error || "Error creating new user!");
  ...
  const checkPassword = (pass) => {
    ...
    const newCheck = {
      isLong: lowerPass.length >= 8,
      includesSymbol: /[\d!@#$%^&*(),.?":{}|<>]/.test(lowerPass),
      includesNameEmail: !lowerPass.includes(lowerName) && !lowerPass.includes(lowerEmail),
    };
    ...
    <div className={styles.registerContainer}>
      ...
      <div className={styles.rightContainer}>
        ...
        <div className={styles.buttonDiv}>
          <button
            type="button"
            className={`${styles.button}${userType === "startup" ? styles.active : ""}`}
            onClick={() => {
              setUserType("startup");
              setStep(1);
              setError("");
        ...
        Startup
          ...
          <button
            type="button"
            className={`${styles.button}${userType === "mentor" ? styles.active : ""}`}
            onClick={() => {
              setUserType("mentor");
              setStep(1);
              setError("");
        ...
        <div className={styles.formContainer}>
          <form onSubmit={step === 1 ? handleContinue : handleSubmit}>
            ...
            <p className={passwordCheck.isLong ? styles.valid : styles.invalid}>
  At least 8 characters
</p>
<p className={passwordCheck.includesSymbol ? styles.valid : styles.invalid}>
  Contains a number or symbol
</p>
<p className={passwordCheck.includesNameEmail ? styles.valid : styles.invalid}>
  Cannot contain your name or email address
</p>
<p>
  Password Strength:{" "}
  {passwordCheck.isLong && passwordCheck.includesSymbol && passwordCheck.includesNameEmail
    ? "Strong"
    : "Weak"}
</p>
</div>
              ...
              <>
                ...
                {userType === 'startup' && (
                  <>
                    ...
                    required />
                    <label>Invite Mentors (optional, comma-separated emails)</label>
                    <input 
                    type="text" 
